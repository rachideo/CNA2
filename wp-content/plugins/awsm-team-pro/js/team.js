        ...
        var settings = $.extend( {}, $.fn.gridderExpander.defaults, options );      
            ...
            function processContent(myself, thecontent){
                ...
                if(settings.showNav){
                    ...
                    htmlcontent += "<a href=\"#\" class=\"awsm-grid-close\">"+settings.closeText+"</a>";
                    htmlcontent += "<a href=\"#\" class=\"awsm-grid-nav prev "+(!prevItem.length?"disabled":"")+"\">"+settings.prevText+"</a>";
                    htmlcontent += "<a href=\"#\" class=\"awsm-grid-nav next "+(!nextItem.length?"disabled":"")+"\">"+settings.nextText+"</a>";
                    htmlcontent += "</div>";
                }     
                ...
                if (!visible) {
                    ...
                    mybloc.find(".awsm-grid-padding").fadeIn(settings.animationSpeed, settings.animationEasing, function () {
                        visible = true;
                        /* CHANGED CALLBACK */
                        if ($.isFunction(settings.onContent)) {
                            settings.onContent(mybloc);
            ...
            /* CLICK EVENT */
            $(document).on("click", ".awsm-grid-list", function (e) {
                e.preventDefault();
                var myself = $(this);
                openExpander(myself);
            });
    ...
    AwsmCustom.init();
	    $( document ).on( 'click', '.awsm-filter-btn', function() {
            ...
            if (!$elem.hasClass('awsm-active-filter')) {
                ...
                $elem.parents('.awsm-grid-wrapper').find('.awsm-grid').stop().fadeTo(100, 0.5);
                $elem.parents('.awsm-grid-wrapper').find(".awsm-grid div.awsm-team-item.awsm-scale-anm").removeClass('awsm-scale-anm').stop().fadeOut(500).promise().done( function() {
                    var $target = $elem.parents('.awsm-grid-wrapper').find("." + selectedClass);
                    if ($target.length > 0) {
                        $target.stop().fadeIn().addClass('awsm-scale-anm');
                    } else {
                        $elem.parents('.awsm-grid-wrapper').find('.awsm-grid div.awsm-team-item').addClass('awsm-scale-anm');
                    }
                    
                    $elem.parents('.awsm-grid-wrapper').find('.awsm-grid').stop().fadeTo(300, 1);
                });
                ...
                $(".current-grid").removeClass("current-grid");
                // Reset drawer styles
                var $current_target = $elem.parents('.awsm-grid-wrapper').find(".awsm-grid-show");
                $current_target.hide();
                var $parent = $elem.parents('.awsm-grid-wrapper').find('.awsm-grid');
